...
def pad_masking(bat):
    ...
    mask[mask != '-1'] = 1
    mask[mask == '-1'] = 0
    mask_attn = mask.astype(int)  # mask for additive attention
    batch[batch == '-1'] = '0'
    return batch, torch.FloatTensor(mask_attn).cuda()
...
def process_batch_data(behavior_data, index):  # padding and masking
    ...
    history = data[:, 1]
    recent = data[:, 2]
    batch_user_history, user_history_mask_attn = pad_masking(history)
    batch_user_short, user_short_mask_attn = pad_masking(recent)
    batch_user_valid = np.array(list(data[:, 3]))
    batch_label = np.zeros(len(index), int)
    return batch_user_history, batch_user_short, user_history_mask_attn, user_short_mask_attn, batch_user_valid, batch_label
...
def evaluate(model, loader, vali_data, epoch):
    ...
    for step, (batch_x, batch_y) in enumerate(loader):  # batch_x: index of batch data
        ...
        user_history_mask_attn = processed_data[2]
        user_short_mask_attn = processed_data[3]
        batch_user_valid = processed_data[4]
        batch_label = processed_data[5]
        # compute output
        batch_predict = model(batch_user_history, batch_user_short, user_history_mask_attn, user_short_mask_attn, batch_user_valid, newsID_categoryID, newsID_subcategoryID, newsID_TitleWordID, newsID_AbstractWordID, newsID_titleEntityId_conf, newsID_abstractEntityId_conf).cuda()
...
def train_and_evaluate(training_data, validation_data):
    ...
    news_encoder = NewsEncoder(num_category, num_subcategory, title_embed_matrix, abstract_embed_matrix, entity_embed_matrix, num_head_text, num_head_entity, text_attn_vector_size, entity_attn_vector_size, news_final_attn_vector_size, news_final_embed_size).cuda()
    model = NRMS_new(news_encoder, final_attn_vector_size).cuda()
