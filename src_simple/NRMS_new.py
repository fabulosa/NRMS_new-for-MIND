...
class NRMS_new(nn.Module):
    def __init__(self, news_encoder, attn_vector_size):
        ...
        self.encode_behavior_size = self.news_encoder.final_embed_size

        self.trans_weight_v = nn.Parameter(torch.FloatTensor(self.encode_behavior_size, attn_vector_size).cuda(), requires_grad=True)
        self.trans_weight_q = nn.Parameter(torch.FloatTensor(attn_vector_size, 1).cuda(), requires_grad=True)
        self.parameters_init()
    ...
    def forward(self, batch_user_history, batch_user_short, user_history_mask_attn, user_short_mask_attn, batch_user_valid, newsID_categoryID, newsID_subcategoryID, newsID_TitleWordID, newsID_AbstractWordID, newsID_titleEntityID_conf, newsID_abstractEntityID_conf):
        ...
        history_news_embeddings = torch.mean(history_news_embeddings * Variable(user_history_mask_attn.unsqueeze(-1), requires_grad=False), dim=1) # mask news
        ...
        short_news_embeddings = torch.mean(short_news_embeddings * Variable(user_short_mask_attn.unsqueeze(-1), requires_grad=False), dim=1)

        concat = torch.cat((history_news_embeddings.unsqueeze(1), history_news_embeddings.unsqueeze(1)), 1)
        attend = F.softmax(torch.matmul(torch.tanh(torch.matmul(concat, self.trans_weight_v)), self.trans_weight_q), dim=1)
        attend = torch.matmul(concat.transpose(1, 2), attend).squeeze(-1)
