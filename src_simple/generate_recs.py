...
def process_batch_data(behavior_data, index):  # padding and masking
    ...
    return batch_user_history, batch_user_short, user_history_mask_attn, user_short_mask_attn, batch_user_valid, mask_softmax, batch_user_impressionID
...
def evaluation(model, vali_data):
    ...
    for step, (batch_x, batch_y) in enumerate(loader):  # batch_x: index of batch data
        ...
        batch_user_mask = processed_data[5]
        batch_user_impressionID = processed_data[6]

        # compute output
        batch_predict = model(batch_user_history, batch_user_short, user_history_mask_attn,
